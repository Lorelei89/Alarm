...
class AlarmAddEditViewController: UIViewController, UITableViewDelegate, UITableViewDataSource{
    ...
    @IBAction func saveEditAlarm(_ sender: AnyObject) {
        ...
        let index = segueInfo.currentCellIndex
        var tempAlarm = Alarm()
        tempAlarm.label = segueInfo.label
        tempAlarm.enabled = segueInfo.enabled
        tempAlarm.setDate(_newDate: date)
        tempAlarm.UUID = UUID().uuidString
        
        if segueInfo.isEditMode {
           // alarmModel.alarms[index] = tempAlarm
            let alarms = alarmModel.allAlarms()
            tempAlarm = alarms[index]
        }
        else {
            var alarms = alarmModel.allAlarms()
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if indexPath.section == 0 {
            ...
            //delete alarm
            var alarms = alarmModel.allAlarms()
            alarms.remove(at: segueInfo.currentCellIndex)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == Id.saveSegueIdentifier {
            ...
            for cell in cells {
                ...
                if sw.tag > segueInfo.currentCellIndex
                {
                    sw.tag -= 1
                }
            }
